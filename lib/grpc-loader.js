...
module.exports = async app => {
  ...
  try {
    ...
    app.coreLogger.debug('[egg-server-server] start on port:' + config.port);
  } catch (e) {
    app.coreLogger.error('[egg-server-server] init proto failure:' + e.message);
  }
};
      ...
      if (stats.isDirectory()) {
        ...
        if (protoServer) {
          ...
          if (ExtendClass) {
            ...
            app.coreLogger.error(`[egg-server-server] the proto file [${photoFilePath}] has no extend js`);
          }
        }
      }
    });
